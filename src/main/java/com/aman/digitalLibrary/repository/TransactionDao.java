...
public class TransactionDao {
    ...
    public int issueBookToStudent(BookTransactionRequest bookTransactionRequest){
        ...
        if ("ISSUE".equalsIgnoreCase(bookTransactionRequest.getRequestType())) {
            ...
            sql = "INSERT INTO transactions (TXN_ID, STUDENT_ID, BOOK_ID, ISSUED_TIME, UPDATED_TIME, COST, TXN_TYPE) VALUES (?, ?, ?, ?, ?, ?, ?)";
            txnType = "ISSUE";
    ...
    private int calculateFine(BookTransactionRequest bookTransactionRequest) {
        // 1. Fetch the original transaction record from the database.
        ...
        Transaction transaction = jdbcTemplate.queryForObject(getDataQuery, new RowMapper<Transaction>(){
            ...
            public Transaction mapRow(ResultSet rs, int rowNum) throws SQLException {
                ...
                // Map the 'ISSUED_TIME' column from the database to the transaction object.
                transaction.setCreatedOn(rs.getTimestamp("ISSUED_TIME"));
                return transaction;
            }
        });

        long issuedTime= transaction.getCreatedOn().getTimestamp().getTime();
        long currentTime = System.currentTimeMillis();

        long diff = TimeUnit.DAYS.convert(currentTime - issuedTime, TimeUnit.MILLISECONDS);
